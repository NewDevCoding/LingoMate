...
import { extractInterceptionRouteInformation, isInterceptionRouteAppPath, INTERCEPTION_ROUTE_MARKERS } from '../shared/lib/router/utils/interception-routes';
import { getNamedRouteRegex } from '../shared/lib/router/utils/route-regex';
import { getSegmentParam, isCatchAll } from '../shared/lib/router/utils/get-segment-param';
 ...
 */ function generateSameLevelHeaderRegex(interceptingRoute, reference) {
    ...
    for(let i = 0; i < segments.length; i++){
        ...
        if (param) {
            ...
            if (!prefixedKey) {
                throw Object.defineProperty(new InvariantError(`No reference found for param: ${param.param} in reference: ${JSON.stringify(reference)}`), "__NEXT_ERROR_CODE", {
                    value: "E868",
                    enumerable: false,
                    configurable: true
                });
 ...
 */ function hasCatchallSiblingAtLevel(appPaths, interceptingRoute) {
    ...
    return appPaths.some((path)=>{
        ...
        for(let i = 0; i < targetDepth; i++){
            ...
            if (INTERCEPTION_ROUTE_MARKERS.some((marker)=>segments[i].startsWith(marker))) {
                return false;
            }
    ...
    if (marker === '(.)') {
        ...
        for(let i = 0; i < segments.length; i++){
            ...
            if (param) {
                ...
                if (!key) {
                    throw Object.defineProperty(new InvariantError(`No reference found for param: ${param.param} in reference: ${JSON.stringify(headerReference)}`), "__NEXT_ERROR_CODE", {
                        value: "E868",
                        enumerable: false,
                        configurable: true
                    });
