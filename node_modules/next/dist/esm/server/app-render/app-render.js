 ...
 */ async function generateDynamicRSCPayload(ctx, options) {
    ...
    let flightData = '';
    const { componentMod: { routeModule: { userland: { loaderTree } }, createMetadataComponents }, getDynamicParamFromSegment, query, requestId, flightRouterState, workStore, url } = ctx;
        ...
        flightData = (await walkTreeWithFlightRouterState({
            ...
            rscHead: /*#__PURE__*/ _jsxs(React.Fragment, {
                children: [
                    ...
                    /*#__PURE__*/ _jsx(Viewport, {}, getFlightViewportKey(requestId)),
                    /*#__PURE__*/ _jsx(Metadata, {}, getFlightMetadataKey(requestId))
                ]
 ...
 */ async function generateDynamicFlightRenderResult(req, ctx, requestStore, options) {
    const { clientReferenceManifest, componentMod, htmlRequestId, renderOpts, requestId, workStore } = ctx;
    ...
    const flightReadableStream = workUnitAsyncStorage.run(requestStore, componentMod.renderToReadableStream, RSCPayload, clientReferenceManifest.clientModules, {
        onError,
...
async function getRSCPayload(tree, ctx, is404) {
    ...
    const { getDynamicParamFromSegment, query, appUsingSizeAdjustment, componentMod: { createMetadataComponents }, url, workStore } = ctx;
...
async function getErrorRSCPayload(tree, ctx, ssrError, errorType) {
    const { getDynamicParamFromSegment, query, componentMod: { createMetadataComponents }, url, workStore } = ctx;
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
function App({ reactServerStream, reactDebugStream, preinitScripts, clientReferenceManifest, ServerInsertedHTMLProvider, nonce, images }) {
    preinitScripts();
    const response = React.use(useFlightStream(reactServerStream, reactDebugStream, clientReferenceManifest, nonce));
...
async function renderToHTMLOrFlightImpl(req, res, url, pagePath, query, renderOpts, workStore, parsedRequestHeaders, postponedState, serverComponentsHmrCache, sharedContext, interpolatedParams, fallbackRouteParams) {
    ...
    const { clientReferenceManifest, serverActionsManifest, ComponentMod, nextFontManifest, serverActions, assetPrefix = '', enableTainting, experimental } = renderOpts;
    ...
    if (process.env.NODE_ENV === 'development' && renderOpts.setIsrStatus && !experimental.cacheComponents) {
    ...
    if (isStaticGeneration) {
        ...
        if (process.env.NODE_ENV === 'development' && renderOpts.setIsrStatus && !experimental.cacheComponents && // Only pages using the Node runtime can use ISR, so we only need to
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, devValidatingFallbackParams) {
    const { assetPrefix, htmlRequestId, nonce, pagePath, renderOpts, requestId } = ctx;
    const { basePath, buildManifest, clientReferenceManifest, ComponentMod, crossOrigin, dev = false, experimental, nextExport = false, onInstrumentationRequestError, page, reactMaxHeadersLength, setReactDebugChannel, shouldWaitOnAllReady, subresourceIntegrityManifest, supportsDynamicResponse } = renderOpts;
    assertClientReferenceManifest(clientReferenceManifest);
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            if (serverIsDynamic || hasFallbackRouteParams) {
                ...
                if (postponed != null) {
                    // Dynamic HTML case
                    metadata.postponed = await getDynamicHTMLPostponedState(postponed, preludeIsEmpty ? DynamicHTMLPreludeState.Empty : DynamicHTMLPreludeState.Full, fallbackRouteParams, resumeDataCache, experimental.cacheComponents);
                } else {
                    // Dynamic Data case
                    metadata.postponed = await getDynamicDataPostponedState(resumeDataCache, experimental.cacheComponents);
                }
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
            if (accessedDynamicData(dynamicTracking.dynamicAccesses)) {
                if (postponed != null) {
                    // Dynamic HTML case.
                    metadata.postponed = await getDynamicHTMLPostponedState(postponed, preludeIsEmpty ? DynamicHTMLPreludeState.Empty : DynamicHTMLPreludeState.Full, fallbackRouteParams, prerenderResumeDataCache, experimental.cacheComponents);
                } else {
                    // Dynamic Data case.
                    metadata.postponed = await getDynamicDataPostponedState(prerenderResumeDataCache, experimental.cacheComponents);
                }
