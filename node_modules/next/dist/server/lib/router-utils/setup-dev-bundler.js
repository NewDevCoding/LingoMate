...
async function startWatcher(opts) {
    ...
    const hotReloader = opts.turbo ? await (async ()=>{
        ...
        return await createHotReloaderTurbopack(opts, serverFields, distDir, resetFetch);
    })() : await (async ()=>{
    ...
    await new Promise(async (resolve, reject)=>{
        ...
        wp.on('aggregated', async ()=>{
            ...
            for (const fileName of sortedKnownFiles){
                const { name } = _path.default.parse(fileName);
                if (name === _constants1.MIDDLEWARE_FILENAME) {
                    hasMiddlewareFile = true;
                }
                if (name === _constants1.PROXY_FILENAME) {
                    hasProxyFile = true;
                }
            }
            if (hasMiddlewareFile) {
                if (hasProxyFile) {
                    throw Object.defineProperty(new Error(`Both "${_constants1.MIDDLEWARE_FILENAME}" and "${_constants1.PROXY_FILENAME}" files are detected. Please use "${_constants1.PROXY_FILENAME}" instead.`), "__NEXT_ERROR_CODE", {
                        value: "E866",
                        enumerable: false,
                        configurable: true
                    });
                }
                _log.warn(`The "${_constants1.MIDDLEWARE_FILENAME}" file convention is deprecated. Please use "${_constants1.PROXY_FILENAME}" instead.`);
            }
            for (const fileName of sortedKnownFiles){
