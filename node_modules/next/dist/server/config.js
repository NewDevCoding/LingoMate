 ...
 */ function assignDefaultsAndValidate(dir, userConfig, silent, configuredExperimentalFeatures, phase) {
    ...
    if ((0, _canaryonlyconfigerror.isStableBuild)()) {
        var _result_experimental7, _result_experimental8;
        // Prevents usage of certain experimental features outside of canary
        if ((_result_experimental7 = result.experimental) == null ? void 0 : _result_experimental7.cacheComponents) {
            throw new _canaryonlyconfigerror.CanaryOnlyConfigError({
                feature: 'experimental.cacheComponents'
            });
        } else if ((_result_experimental8 = result.experimental) == null ? void 0 : _result_experimental8.turbopackFileSystemCacheForBuild) {
            throw new _canaryonlyconfigerror.CanaryOnlyConfigError({
    ...
    if (result.experimental.ppr) {
        throw new _harddeprecatedconfigerror.HardDeprecatedConfigError(`\`experimental.ppr\` has been merged into \`experimental.cacheComponents\`. The Partial Prerendering feature is still available, but is now enabled via \`experimental.cacheComponents\`. Please update your ${configFileName} accordingly.`);
    }
    ...
    if (result.experimental.cacheComponents && ((_userConfig_experimental1 = userConfig.experimental) == null ? void 0 : _userConfig_experimental1.rdcForNavigations) === undefined) {
...
function enforceExperimentalFeatures(config, options) {
    ...
    (config.experimental.clientParamParsing === undefined || isDefaultConfig && !config.experimental.clientParamParsing)) {
        ...
        if (configuredExperimentalFeatures) {
            addConfiguredExperimentalFeature(configuredExperimentalFeatures, 'clientParamParsing', true, 'enabled by `__NEXT_EXPERIMENTAL_CACHE_COMPONENTS`');
        }
    }
