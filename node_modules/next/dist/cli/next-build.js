...
const nextBuild = (options, directory)=>{
    ...
    const { debug, debugPrerender, experimentalDebugMemoryUsage, profile, mangling, experimentalAppOnly, experimentalBuildMode, experimentalUploadTrace } = options;
    let traceUploadUrl;
    ...
    return (0, _build.default)(dir, profile, debug || Boolean(process.env.NEXT_DEBUG_BUILD), debugPrerender, !mangling, experimentalAppOnly, bundler, experimentalBuildMode, traceUploadUrl).catch((err)=>{
        if (experimentalDebugMemoryUsage) {
